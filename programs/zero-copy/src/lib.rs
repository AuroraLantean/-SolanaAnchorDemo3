...
pub struct CreateBar<'info> {
    ...
    authority: Signer<'info>,
    foo: AccountLoader<'info, Foo>,
    /// CHECK: "system_program", for making a new associated zero copy account, is unsafe, but is not documented. explaining here why no checks through types are necessary
